...
void app_init(void)
{
    gpio_port_config(PORT_B, 0x7F); //display-1
    gpio_port_config(PORT_L, 0xFF); //display-2
    gpio_port_config(PORT_C, 0xFF); // digit selction
...
void display_adc_and_voltage(uint16_t adc)
{
   ...
   /* digit-1*/ 
    gpio_port_write(PORT_C, 0x00); //initally all off
    gpio_port_write(PORT_B, seg_code[adc_d1]);// adc value
    voltage_pattern = seg_code[v_d1] & 0x7F; //to have decimal point (bit7=0)
    gpio_port_write(PORT_L, voltage_pattern);
    gpio_port_write(PORT_C, 0x11);
    small_delay();

   /* digit-2 */
    gpio_port_write(PORT_C, 0x00);

    gpio_port_write(PORT_B, seg_code[adc_d2]);
    gpio_port_write(PORT_L, seg_code[v_d2]);
    gpio_port_write(_PORT_C, 0x22);
    ...
    gpio_port_write(PORT_B, seg_code[adc_d3]);
    gpio_port_write(PORT_L, seg_code[v_d3]);
    gpio_port_write(PORT_C, 0x44);
    ...
    gpio_port_write(PORT_B, seg_code[adc_d4]);
    gpio_port_write(PORT_L, seg_code[v_d4]);
    gpio_port_write(PORT_C, 0x88);
